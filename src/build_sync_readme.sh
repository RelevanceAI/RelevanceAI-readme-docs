...
if $DEBUG_MODE; then
	...
	python src/build/update_asset_ref.py -d -p $PWD -pn $PIP_PACKAGE_NAME -v $GIT_BRANCH_NAME
else
	python src/build/update_asset_ref.py -p $PWD -pn $PIP_PACKAGE_NAME -v $GIT_BRANCH_NAME
fi

CYAN "=== Updating semver ref to $README_VERSION ==="

if $DEBUG_MODE; then
	python src/build/update_semver_ref.py -d -p $PWD -pn $PIP_PACKAGE_NAME -v $GIT_BRANCH_NAME_VERSION
else
	python src/build/update_semver_ref.py -p $PWD -pn $PIP_PACKAGE_NAME -v $GIT_BRANCH_NAME_VERSION
fi
...
###
if $DEBUG_MODE; then
	python src/build/build_docs.py  -d -c -p $PWD -pn $PIP_PACKAGE_NAME -v $GIT_BRANCH_NAME
else
	python src/build/build_docs.py -c -p $PWD -pn $PIP_PACKAGE_NAME -v $GIT_BRANCH_NAME
fi

# CYAN "=== Converting notebooks to Markdown $GIT_BRANCH_NAME ==="
# ### Converts *ipynb  to *.md not in _notebooks
# if $DEBUG_MODE; then
# 	python src/readme/nbconvert_rdmd_preprocessor.py -d  -p $PWD -v $GIT_BRANCH_NAME
# else
# 	python src/readme/nbconvert_rdmd_preprocessor.py  -p $PWD -v $GIT_BRANCH_NAME
# fi

# CYAN "=== Updating config $GIT_BRANCH_NAME with new files ==="
# ### Updates ReadMe with new files in docs
# if $DEBUG_MODE; then
# 	python src/config/sync.py -d --method "update"   -p $PWD -v $GIT_BRANCH_NAME
# else
# 	python src/config/sync.py --method "update"  -p $PWD -v $GIT_BRANCH_NAME
# fi
