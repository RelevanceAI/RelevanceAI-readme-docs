...
# from readme_sync.utils.find_replace import file_find_replace, notebook_find_replace
...
def notebook_find_replace(
    ...
    for cell in notebook_json["cells"]:
        # logging.debug(find_sent_regex, str(cell["source"]))
        ...
        if bool(re.search(find_sent_regex, str(cell["source"]))):
            ...
            for i, cell_source in enumerate(cell["source"]):
                if bool(re.search(find_str_regex, cell_source)):
                    ...
                    cell["source"][i] = cell_source.strip()
                else:
                    continue
